  ...
  public class HWebSynchronizeHandler : IHttpHandler, System.Web.SessionState.IRequiresSessionState
  {
    ...
    public void ProcessRequest(HttpContext context)
    {
      ...
      if (context.Request.Url.AbsolutePath.EndsWith(".js"))
      {
        ...
        watcher.Start();
        HtmlResult<HElement> result = handler(prevState, json_commands ?? new JsonData[0], new HContext(handlerPair.Key, context));
      ...
      else
      {
        ...
        var result = handler(lastState, new JsonData[0], new HContext(handlerPair.Key, context));
        var page = result.Html;
